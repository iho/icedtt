...
impl Application for TimeTracker {
    type Message = Message;
    type Theme = Theme;
    type Executor = iced::executor::Default;
    type Flags = ();

    fn new(_flags: ()) -> (TimeTracker, Command<Message>) {
        (
            TimeTracker::Loading,
            Command::batch(vec![
    ...
    fn title(&self) -> String {
        let dirty = match self {
            TimeTracker::Loading => false,
            TimeTracker::Loaded(state) => state.dirty,
        };

        format!("Tasks{} - Iced", if dirty { "*" } else { "" })
    }
        ...
        match self {
            TimeTracker::Loading => {
                match message {
                    ...
                    Message::Loaded(Err(_)) => {
                        *self = TimeTracker::Loaded(State::default());
                    }
                    _ => {}
                }
            ...
            TimeTracker::Loaded(state) => {
                ...
                let command = match message {
                    ...
                    Message::FontLoaded(_) | Message::Loaded(_) => Command::none(),
                    Message::ShowReport => Command::none(),
                    Message::StartTask(str) => {
                        let task = state.tasks.iter_mut().find(|task| task.description == str);
                        if let Some(task) = task {
                            task.update(TaskMessage::Started);
                        }
                        Command::perform(
                            SavedState {
                    ...
                    Message::StopTask(str) => {
                        let task = state.tasks.iter_mut().find(|task| task.description == str);
                        if let Some(task) = task {
                            task.update(TaskMessage::Stopped);
                        }
                        Command::perform(
                            SavedState {
                                input_value: state.input_value.clone(),
    ...
    fn view(&self) -> Element<Message> {
        match self {
            TimeTracker::Loading => loading_message(),
            TimeTracker::Loaded(State {
                input_value,
                filter,
                tasks,
        ...
        match &self.state {
            TaskState::Idle => {
                let checkbox = checkbox(&self.description, self.completed, TaskMessage::Completed)
                    .width(Length::Fill);
                // .text_size(text::StyleSheet::Advanced);

                let start_button = if !self.is_active {
                    row![button("Start")
                        .on_press(TaskMessage::Started)
                        .padding(10)
                        .style(theme::Button::Text)]
                } else {
                    row![
                        button("in progress").padding(10).style(theme::Button::Text),
                        button("Stop")
                            .on_press(TaskMessage::Stopped)
                            .padding(10)
                            .style(theme::Button::Text)
                    ]
                };
                dbg!(self.time_entries.len());
                row![
                    checkbox,
                    start_button,
                    button(edit_icon())
...
fn tachometer_alt_icon() -> Text<'static> {
    icon('\u{F3FD}')
}
...
impl SavedState {
    ...
    async fn load() -> Result<SavedState, LoadError> {
        ...
        let res = serde_json::from_str(&contents).map_err(|_| LoadError::Format);
        dbg!(res.clone());
        res
    }
    ...
    async fn save(self) -> Result<(), SaveError> {
        ...
        {
            tokio::fs::create_dir(&path.parent().unwrap())
                .await
                .map_err(|_| SaveError::File)?;
